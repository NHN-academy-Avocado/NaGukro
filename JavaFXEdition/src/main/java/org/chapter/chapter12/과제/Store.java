...
public class Store {
    ...
    public void enter() throws InterruptedException {
        ...
        System.out.println("고객 입장. 현재 매장 내 고객 수: " + storeSemaphore.availablePermits());
    }

    public void exit() {
        storeSemaphore.release(); // 매장을 나갈 때 세마포어 반환
        System.out.println("고객 퇴장. 현재 매장 내 고객 수: " + storeSemaphore.availablePermits());
    }
    ...
    public synchronized void sell(String item) throws InterruptedException {
        while (inventory.get(item) >= MAX_CAPACITY) {
            wait(); // 해당 물품이 가득 차면 대기
        }
        inventory.put(item, inventory.get(item) + 1); // 물품 입고
